...
class WeatherClient:
    ...
    def refresh(self, on_progress=None) -> dict:
        params = (
            "latitude={}&longitude={}"
        ...
        if on_progress and getattr(self.http_client, "supports_progress", False):
            response = self.http_client.get(url, on_progress=on_progress)
        else:
